...
- **Language.** A Dutch list will not retrieve German, French, Polish or Greek cases. Each
  ...
  jurisdiction's courts. The number of a Union instrument is written the same in every
  language; its title and short name are not, and the bracketed form of a number carries a
  language's own token (`(EU)`, `(UE)`, `(ΕΕ)`). Multilingual jurisdictions (Belgium,
  Luxembourg, Malta, Cyprus, Ireland, and the EU itself) carry several languages in one
  list.
...
are hand-curated. Everything made under the Union base instruments — 2,245 acts CELLAR
...
typed from memory would have a recall floor. At version 1.0.0 the lists hold 2,276
instruments for the Netherlands and 2,254 for the EU.
...
forms a court writes. Number/year numbers (regulations before 2015: `1107/2009`) are matched
bare. Year/number numbers (every directive and decision, and regulations from 2015) are
matched only in bracketed, suffixed and worded forms — `(EU) 2017/2324`, `2009/128/EC`,
`Richtlijn 2009/128` — never bare, because `2009/128` is how Dutch case-law reporters cite
judgments (*NJ 2009/128*). That is the only exception the method needs, and each
jurisdiction document records it (§2.10).
