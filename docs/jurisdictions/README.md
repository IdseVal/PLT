...
which source, against which legislation, and what the tracker is known not to hold.

| Code | Jurisdiction | Document | Legislation list | Source last checked |
| --- | --- | --- | --- | --- |
| `NL` | Netherlands | [`nl.md`](nl.md) | `data/legislation/nl.json` | 4 August 2026 |
| `EU` | European Union | [`eu.md`](eu.md) | `data/legislation/eu.json` | 4 August 2026 |
...
3. **How cases are selected** — the legislation list, and what a test run over a real
   period measured.
...
1. Copy `TEMPLATE.md` to `<code>.md`, lower case, matching the legislation list file name.
...
4. Write the legislation list (`data/legislation/README.md`): the national instruments from
   what the jurisdiction's courts cite, the Union base instruments in that language's
   citation forms, and the generated acts under them. Run it over the store or a sample
   period, and report what it measured in §3.
5. Record any exception in §4 with its cost. Legislation lists are curated by the content
